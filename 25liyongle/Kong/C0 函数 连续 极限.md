<span style="color: LightSkyBlue;">~</span> : <span style="color: LightSkyBlue;"><span style="color: LightSkyBlue;">函数</span></span>
...
设 \(x\) 和 \(y\) 是两个变量, \({\color{gray}D}\) 是一个给定的<span style="color: gray;">非空</span>数集。如果对于每个 \(x \in {\color{gray}D}\)，按照一定的____ \(f\) 总有一个确定的数值 \(y\) 和它<span style="color: GreenYellow;">对应</span>，则称 \(y\) 是 \(x\) 的____，记为
\[
y=f(x), x \in D
\]
其中 \(x\) 称为____, \(y\) 称为____, \({\color{gray}D}\) 称为____的____, 记作 \(D_{f}\)，即 \(D_{f}={\color{gray}D}\)。____ 值 \(f(x)\) 的全体所构成的集合称为____ \(f\) 的____, 记作 \(R_{f}\) 或 \(f(D)\)，即
\[
...
分式的____不为 \(0\)
\(\sqrt[m]{x}(x \geqslant 0)\), \(\log _{x} x(x>0)\)。  
...
<span style="color: GreenYellow;">对应</span>法则 <span style="color: LightSkyBlue;">函数</span></li>
    <li>

自变量 因变量 
<span style="color: LightSkyBlue;">函数</span> 定义域</li>
    <li>

<span style="color: LightSkyBlue;">函数</span> <span style="color: LightSkyBlue;">函数</span>值域</li>
    <li>

<span style="color: LightSkyBlue;">函数</span></li>
    <li>

定义域 <span style="color: GreenYellow;">对应</span>法则</li>
    <li>

<span style="color: LightSkyBlue;">函数</span> 定义域 <span style="color: GreenYellow;">对应</span>法则<span style="color: LightSkyBlue;">函数</span></li>
    <li>
...
\end{array}\right.
\]
（4） <span style="color: Gold;">符号</span>____ 也是分段____
\[
...
\end{array}\right.
\]
（5）<span style="color: gray;">取整</span>____ \(y=[x]\) 也是分段____。  
\([x]\) 表示不超过 \(x\) 的____。  
...
<span style="color: LightSkyBlue;">函数</span> 定义域</li><span style="color: GreenYellow;">对应</span>法则</li>
    <li>

<span style="color: LightSkyBlue;">函数</span><span style="color: LightSkyBlue;">函数</span></li>
    <li>

<span style="color: LightSkyBlue;">函数</span><span style="color: LightSkyBlue;">函数</span><span style="color: LightSkyBlue;">函数</span></li>
    <li>

<span style="color: LightSkyBlue;">函数</span><span style="color: LightSkyBlue;">函数</span>定义域</li>
    <li>
...
<span style="color: LightSkyBlue;">函数</span><span style="color: LightSkyBlue;">函数</span>最大整数</li>
    <li>
...
设____ \(y=f(u)\) 的____为 \({\color{gray}D}_{f}\), ____ \(u=g(x)\) 的____为 \(D_{g}\), ____ 为 \({\color{gray}R}_{g}\), 
- 若 \(D_{f} \cap R_{g} \neq \varnothing\), 则称 ____ \(y=f[g(x)]\) 为____ \(y=f(u)\) 与 \(u=g(x)\) 的复合____

其____为 \(\left\{x \mid x \in D_{g}, g(x) \in D_{f}\right\}\)。
...
<span style="color: LightSkyBlue;">函数</span>定义域 值域</li>
    <li>
...
###  <span style="color: silver;">4. 反<span style="color: LightSkyBlue;">~</span> ✓

<ul>

设____ \(y=f(x)\) 的____为 \({\color{gray}D}\), ____为 \({\color{gray}R}_{y}\)。

- 若对任意 \(y \in {\color{gray}R}_{y}\), 有 <u>唯一确定</u> 的 \(x \in {\color{gray}D}\), 使得 \(y=f(x)\), 则记为 \(x=f^{-1}(y)\)
- 称其为____ \(y=f(x)\) 的反____。
...
1. 不是每个____都有反____
   ...
   - 如 \(y=x^{3}\) 有反____，而 \(y=x^{2}\) 没有反____。 
  
2. 单调____一定有反____，但反之则不然
   - 如 \(f(x)=\left\{\begin{array}{cc}x, & 0 \leqslant x<1, \\ 3-x, & 1 \leqslant x \leqslant 2\end{array}\right.\) 有反____, 但不单调
3. 通常将 \(y=f(x)\) 的反____ \(x=f^{-1}(y)\) 写成 \(y=f^{-1}(x)\)。
   - 在同一直角坐标系中, \(y=f(x)\) 和 \(x=f^{-1}(y)\) 的图形____
   - \(y=f(x)\) 和 \(\color{gray}y=f^{-1}(x)\) 的图形关于直线 \(y=x\) ____
...
<span style="color: LightSkyBlue;">函数</span><span style="color: LightSkyBlue;">函数</span><span style="color: LightSkyBlue;">函数</span><span style="color: LightSkyBlue;">函数</span></li>
    <li>
...
设有关系式 \(F(x, y)=0\)，若 \(\forall x \in {\color{gray}D}\)，存在 <u>唯一确定</u> 的 \(y\) 满足 \(F(x, y)=0\) 与 \(x\) 相<span style="color: GreenYellow;">对应</span>，由此确定的 \(y\) 与 \(x\) 的____关系 \(y=y(x)\) 称为由方程 \(F(x, y)=0\) 所确定的隐____。
...
设____ \(y = f(x)\) 的____ \({\color{gray}D}\) 关于原点对称 (即若 \(x \in {\color{gray}D}\)，则有 \(-x \in {\color{gray}D}\))，如果对于任一 \(x \in {\color{gray}D}\)，恒有

\[ f(-x) = f(x) \]

则称 \(f(x)\) 为 \({\color{gray}D}\) 上的____；如果对于任一 \(x \in {\color{gray}D}\)，恒有

\[ f(-x) = -f(x) \]

则称 \(f(x)\) 为 \({\color{gray}D}\) 上的____。
...
偶<span style="color: LightSkyBlue;">函数</span></li>
    <li>

奇<span style="color: LightSkyBlue;">函数</span></li>
    <li>
...
周期<span style="color: LightSkyBlue;">函数</span></li>
    <li>
...
周期<span style="color: LightSkyBlue;">函数</span>的运算性质</li>
  </ul>
</details>
</div>

###  <span style="color: silver;">4. 有界性
...
有界<span style="color: LightSkyBlue;">函数</span></li>
    <li>

无界<span style="color: LightSkyBlue;">函数</span></li>
    <li>
...
- （数列）设 \(\lim _{n \rightarrow \infty} x_{n}=A\)。  
  ...
  【注】1. 注意结论1. 中是____不等号（\(>\) 或 \(<\)）；而2. 中是____不等号（\(\geqslant\) 或 \(\leqslant\)）。  
  2.  结论2. 中若条件改为：当 \(n>N\) 时，\(x_{n}>0\)（或 \(x_{n}<0\)），则结论不变：仍有 \(A \geqslant 0\)（或 \(A \leqslant 0\)。如 \(\lim _{n \rightarrow \infty} \frac{1}{n}=0\)。  
- （____）设 \(\lim _{x \rightarrow x_{0}} f(x)=A\)。  
  ...
  【注】1. 结论2. 中若条件改为：\(f(x)>0\)（或 \(f(x)<0\)），则结论不变：有 \(A \geqslant 0\)（或 \(A \leqslant 0\)。2.  对于自变量 \(x\) 的其他____过程，也有类似的____性质。
...
####  <span style="color: silver;">4. 无穷大量与无界变量的关系
...
【注】  
- 条件1. 只要在____过程 \(x \rightarrow x_{0}\) 附近成立即可。  
- 换为其他____过程也有相应的夹逼准则。  
- 对于数列也有类似的夹逼准则。  
...
连续<span style="color: LightSkyBlue;">函数</span></li>
    <li>
