...
<span style="color: GreenYellow;">对应</span>法则</li>
    <li>
...
设有关系式 \(F(x, y)=0\)，若 \(\forall x \in D\)，存在唯一确定的 \(y\) 满足 \(F(x, y)=0\) 与 \(x\) 相<span style="color: GreenYellow;">对应</span>，由此确定的 \(y\) 与 \(x\) 的____关系 \(y=y(x)\) 称为由方程 \(F(x, y)=0\) 所确定的隐____。
